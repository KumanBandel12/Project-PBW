...
function getTugas($db, $user_id) {
    try {
        $query = "SELECT * FROM tugas WHERE user_id = :user_id ORDER BY date_start DESC";
...
function updateTugas($db, $data, $user_id) {
    try {
        ...
        $startDate = $data['startDate'] ?? null;
        $targetDate = $data['targetDate'] ?? null;
        // Konversi nilai 'completed' dari frontend (true/false) ke boolean (1/0) untuk DB
