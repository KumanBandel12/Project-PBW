...
// --- PERBAIKAN: Membaca input JSON dari body request ---
$input = json_decode(file_get_contents('php://input'), true);
$method = $_SERVER['REQUEST_METHOD'];

// Menentukan parameter dari sumber yang benar (GET atau JSON body)
...
function getTrashItems($db, $user_id) {
    try {
        // Query dasar untuk mengambil SEMUA item sampah milik user
        ...
        // Lakukan filtering di sini (dalam PHP) JIKA ada search term
        if (!empty($_GET['search'])) {
            $search_term = strtolower($_GET['search']);
            $filteredItems = array_filter($allItems, function($item) use ($search_term) {
                // Mencari secara case-insensitive pada judul
                return stripos(strtolower($item['title']), $search_term) !== false;
            });
            // Mengembalikan hasil yang sudah difilter
            echo json_encode(["success" => true, "data" => array_values($filteredItems)]);
        } else {
            // Jika tidak ada pencarian, kembalikan semua item
